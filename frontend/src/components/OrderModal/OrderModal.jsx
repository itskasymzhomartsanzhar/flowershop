...
useEffect(() => {
  ...
  const getStatusStep = (status, isPickup) => {
    const normalized = status || 'PLACED';
    const assemblingStatuses = ['ASSEMBLING', 'ASSEMBLING_STARTED', 'ASSEMBLING_DONE'];
    if (isPickup) {
      if (['READY_FOR_PICKUP', 'ISSUED', 'DELIVERED'].includes(normalized)) return 3;
      if (assemblingStatuses.includes(normalized)) return 2;
      return 1;
    }
    if (normalized === 'DELIVERED') return 4;
    if (['COURIER_PICKED', 'DELIVERING', 'ONTHEWAY'].includes(normalized)) return 3;
    if (assemblingStatuses.includes(normalized)) return 2;
    return 1;
  };

  const currentStep = getStatusStep(order.status, order.is_pickup);
        ...
        <div className="order-modal__scrollable">
          ...
          <div className="order-modal__status-section">
            ...
            <div className="order-modal__timeline">
              <div className={`order-modal__status ${isStepActive(1) ? 'order-modal__status--active' : ''}`}>
                <div className="order-modal__status-dot"></div>
                <span className="order-modal__status-label">Оформление</span>
              </div>

              <div className="order-modal__status-line"></div>

              <div className={`order-modal__status ${isStepActive(2) ? 'order-modal__status--active' : ''}`}>
                <div className="order-modal__status-dot"></div>
                <span className="order-modal__status-label">Сборка</span>
              </div>

              <div className="order-modal__status-line"></div>

              <div className={`order-modal__status ${isStepActive(3) ? 'order-modal__status--active' : ''}`}>
                <div className="order-modal__status-dot"></div>
                <span className="order-modal__status-label">{order.is_pickup ? 'В пункте выдачи' : 'В пути'}</span>
              </div>

              {!order.is_pickup && (
                <>
                  <div className="order-modal__status-line"></div>
                  <div className={`order-modal__status ${isStepActive(4) ? 'order-modal__status--active' : ''}`}>
                    <div className="order-modal__status-dot"></div>
                    <span className="order-modal__status-label">Доставлен</span>
                  </div>
                </>
              )}
            </div>
            ...
            <div className="order-modal__delivery-section">
              ...
              <div className="order-modal__info-card">
                ...
                <div className="order-modal__info-content">
                  ...
                  <span className="order-modal__info-value">{order.delivery_address}</span>
                </div>
              </div>
            </div>
          )}
